...
この記事では，進化計算の一種である遺伝的プログラミング(Genetic Programming:GP)@<bib>{Koza}を用い，システムトレードの第一歩として，FXで稼げそうな投資戦略を作り出すことを目的とする．
...
GPを使ってテクニカル指標を組み合わせて投資戦略を作ろうとする試み@<bib>{turing},@<bib>{Zhu}はいくつかあるが，いずれも@<img>{ind_origin}の決定木のような個体表現が多い印象を受ける．しかし，個人的にこの表現が気に食わない．
理由としては，
...
ここではプロフィットファクター(Profit Factor;PF)を用いる@<fn>{fn:PF}．PFは次式で定義される．
//footnote[fn:PF][最近気づいたが，PFって日本以外ではほとんど使われていないっぽい？]
...
重要だと思うものを2つピックアップする(@<bib>{DayTrade}を特に参考としている)．
...
などの欲がどうしても出てしまい判断が狂ってしまいがちである．自動トレードシステムを構築すれば，そういった要素を一切排除できるのも魅力である@<fn>{fn:memo}．
...
=== 使用するデータ
使用するデータは，トレードシミュレーターを販売しているForex Testerから取得できるUSD/JPYの2007年から2016年のデータ@<bib>{forex}とする．
サイトから取得できるデータは1分足データであるが，これを10分足に加工し，この時間軸でテクニカル指標のデータを生成した．10分足データにした理由は，ノイズを除去するためである．
...
#@#最大トレード数 	 年720回
#@#最小トレード数 	 年120回
#@#テクニカル指標数	移動平均線，ボリンジャーバンドなど21個
#@#複製選択方式 	 ルーレット選択
#@#評価基準	 PF
#@#//}

//tabletabooularraw[param][実験設定]{
@<b>{パラメータ} | @<b>{値}
================|===================
最大世代数 |  30
集団数     | 3000
