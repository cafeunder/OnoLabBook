...
進化計算のひとつである遺伝的アルゴリズムを使って解く手法について解説します．
...
== 遺伝的アルゴリズム
@<b>{遺伝的アルゴリズム (Genetic Algorithm: GA) }は，生物の進化過程を模倣した，汎用的近似解法（@<b>{メタ戦略}）@<fn>{fn_meta}のひとつです．
この章では，最もナイーブなGAについて簡単に説明していきます．
...
探索が停滞した時点で，後述するBlock2戦略に切り替える手法が提案されています．
...
4. 交叉ペア@<m>{(p_{\rm A\}, p_{\rm B\})}を@<m>{(x_i,x_{i+1\}) \ (i=1,2,\dots,n_{\rm pop\})}として，以下の操作を行う．
...
=== 設定
GA-EAXの設定として，集団サイズ@<m>{n_{\rm pop\}=1,000}，子個体生成数@<m>{n_{\rm ch\}=30}とします．
...
@<img>{mkgaeax}に経路を示します．2-Optで求めた@<img>{mk2opt}の経路と比較すると，長いエッジが消えていることが分かります．
もちろんこれは，今回の実験設定によるもので，インスタンスに合わせてパラメータをチューニングすることで，
更なる既知最良解を発見できる可能性があります．
//footnote[fn_best][2017年7月28日現在．]

//image[mkgaeax][mk39393の既知最良解（経路長5,853,137）]
...
=== 謝辞
本来なら巻末に書くべきですが，個人的なこともあるのでここに書きます．
まず初めに，本誌を刊行する上で多大なる協力をいただいた，SIGCOWWのlrks@<fn>{fn_lrks}君にお礼申し上げます．
lrks君には，SIGCOWWの組版システムの使用を快く承諾してもらったたほか，
コミケ参加から印刷所への入稿まで，様々な場面でアドバイスを貰いました．
本誌の刊行はlrks君なしには実現しなかったことです．本当にありがとうございました．
//footnote[fn_lrks][@__lrks__]

また，C92にて委託頒布を引き受けて頂いたSIGCOWW@<fn>{fn_sig}のメンバーにもお礼申し上げます．
なお，SIGCOWWのCOSMIC L0（コズミックレイヤゼロ）のVol.2からは僕も参加していますので，
よろしければそちらも御覧ください！（宣伝）
//footnote[fn_sig][@<href>{https://sigcoww.herokuapp.com/}]

最後に，本誌の記事を書いてくれたRosenblock Chainersのメンバーにお礼申し上げます．
技術同人誌を出す，という話は今年（2017年）の研究室旅行でふんわり決まったことですが，
その約1ヶ月後にコミケ委託参加の話を切り出して随分焦らせてしまいました．
正直，ちゃんと同人誌の形になるか心配だったのですが，メンバーの協力もあり無事に発刊することができました．
